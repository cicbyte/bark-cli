#!/usr/bin/env python3
"""本地构建脚本 — 交叉编译 bark-cli 并可选 UPX 压缩"""
...
LDFLAGS_PREFIX = "github.com/cicbyte/bark-cli/cmd/version"
...
def build_target(goos, goarch, version, commit, build_time):
    ...
    output_name = f"bark-cli_{goos}_{goarch}{ext}"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="bark-cli 本地构建脚本")
    ...
    print(f"bark-cli {version} | commit: {commit[:8]} | {build_time}")
    print()
